...
export default class Analytics {
  ...
  constructor(propertyId, additionalParameters = {}, options = defaultOptions) {
    ...
    this.parameters = {
      an: Updates.manifest.name,
      aid: Updates.manifest.slug,
      av: Updates.manifest.version,
      sr: `${width}x${height}`,
      ...additionalParameters,
    };

    this.promiseGetWebViewUserAgentAsync = getWebViewUserAgent(options).then(
      (userAgent) => {
  ...
  send(hit) {
    ...
    const url = `https://www.google-analytics.com/collect?tid=${
      ...
      this.clientId
    }&${hit.toQueryString()}&${params}&${customDimensions}&${customMetrics}&z=${Math.round(
      Math.random() * 1e8
    )}`;
